...
- `npm run build` runs `npm run sitemap`, `npm run build:search`, `next build`, and `npm run prerender` in sequence.
...
- Blog metadata is in `src/data/blogPosts.ts`; blog bodies live in `src/views/BlogPost.tsx` with per-slug blocks.
...
- Title policy is intentionally minimal: the homepage uses `rebelwithsuz.com`, static pages use short labels from `src/data/seoRoutes.ts`, and blog/recipe detail pages use the post or recipe title without a site-name suffix.
...
- Register the route in `app/` with a `page.tsx` file that imports the view component.
...
- For blog updates: add metadata in `src/data/blogPosts.ts` and the post body in `src/views/BlogPost.tsx`.
